...
public class GlideCircleCropWithBorderTransform extends BitmapTransformation {
    ...
    public GlideCircleCropWithBorderTransform(Context context, String url, float borderWidth, int borderColor, int heightPX, int widthPx) {
        this.url = url;
        mBorderWidth = Resources.getSystem().getDisplayMetrics().density * borderWidth;
        mBorderWidth = UIUtil.dip2px(context, borderWidth);
    ...
    private Bitmap circleCrop(BitmapPool pool, Bitmap source, int outWidth, int outHeight) {
        ...
        Bitmap result;
        Bitmap output = pool.get(width, width, Bitmap.Config.ARGB_8888);
...
//        }
    ...
    private byte[] getCacheKeyBytes() {
        try {
            return url.getBytes(Charset.defaultCharset());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
